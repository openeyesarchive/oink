...
public class PatientResourceProvider implements IResourceProvider {
	...
	private Map<String, Patient> identifierToPatient = new HashMap<String, Patient>();
	...
	public PatientResourceProvider() {
		...
		addNewVersion(patient, myNextId++);
	...
	private void addNewVersion(Patient thePatient, Long theId) {
		...
		for(IdentifierDt id : thePatient.getIdentifier()) {
			if(!identifierToPatient.containsKey(id.getValueAsQueryToken())) {
				identifierToPatient.put(id.getValueAsQueryToken(), thePatient);
			}
		}
	...
	@Search()
	public List<Patient> findPatientsByIdentifier(@RequiredParam(name = Patient.SP_IDENTIFIER) StringDt theIdentifier) {
		LinkedList<Patient> retVal = new LinkedList<Patient>();

		String value = theIdentifier.getValueAsString();
		if(identifierToPatient.containsKey(value)) {
			retVal.add(identifierToPatient.get(value));
		}
	...
	public Patient readPatient(@IdParam IdDt theId) {
		...
		try {
			retVal = myIdToPatientVersions.get(theId.getId().asLong());
	...
	public Patient vreadPatient(@IdParam IdDt theId, @VersionIdParam IdDt theVersionId) {
		...
		try {
			versions = myIdToPatientVersions.get(theId.getId().asLong());
		} catch (NumberFormatException e) {
			/*
			 * If we can't parse the ID as a long, it's not valid so this is an unknown resource
			 */
