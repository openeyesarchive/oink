...
/**
 * 
 * Builds a {@link FacadeRoutingService} using information in the adapters
 * properties file.
 * 
 * facade.mapping.1.service (Optional) Used to route to different end-systems
 * facade.mapping.1.resource (Optional) E.g. Patient, Practitioner or
 * Organization facade.mapping.1.routingKey (Mandatory) The rabbit routing key
 * facade.mapping.1.method (Optional) GET, POST or PUT etc
 * facade.mapping.2.service facade.mapping.2.resource .. ... .. ..
 * 
 * @author Oliver Wilkie
 */
public class FacadeRoutingServiceFactory {

	private static final Logger log = LoggerFactory.getLogger(FacadeRoutingServiceFactory.class);
	...
	public FacadeRoutingService createInstance() {
		...
		for (int i = 1; i <= numOfMappings; i++) {
			...
			if (resource != null && !resource.equals("*") && !isReservedResourceWord(resource)) {
				throw new InvalidParameterException("A resource must be a recognised Resource name e.g. Patient  (case-sensitive)");
			}

			String method = mappingCfg.getString("method");
			HttpMethod methodEnum;
			if (method != null && !method.equals("*") && !isValidHttpMethod(method)) {
				throw new InvalidParameterException("A resource must be a recognised method verb e.g. POST,GET,PUT  (case-sensitive)");
			...
			HttpMapperEntry<RabbitRoute> entry = new HttpMapperEntry<RabbitRoute>(uri.toString(), methodEnum, route);
